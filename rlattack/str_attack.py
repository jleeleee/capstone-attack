...
_logger = utils.create_logger("cleverhans.attacks.str_attack_l2")
...
class StrAttackL2(Attack):
  ...
  def __init__(self, model, sess, dtypestr='float32', **kwargs):
    ...
    super(StrAttackL2, self).__init__(model, sess, dtypestr, **kwargs)
  ...
  def generate(self, x, **kwargs):
    ...
    attack = LADMMSTL2(self.sess, self.model, self.batch_size, self.confidence,
...
class LADMMSTL2:
    def __init__(self, sess, model, batch_size=1, confidence=CONFIDENCE,
                 ...
                 abort_early=ABORT_EARLY, initial_const = INITIAL_CONST,clip_min=0.0,clip_max=1.0,num_labels=None,shape=None,image_size=32,stride=2,filter_size=2,num_channels=4,print_every = 100,
    ...
    def gradient_descent(self, sess, model):
        ...
        if self.shape is None:
            ...
            shape = tuple([batch_size] + list(self.shape))
        ...
        tlab = tf.Variable(np.zeros((batch_size, self.num_labels)), dtype=tf.float32)
        const = tf.Variable(np.zeros(batch_size), dtype=tf.float32)

        # and here's what we use to assign them
        assign_timg = tf.placeholder(tf.float32, shape)
        assign_tlab = tf.placeholder(tf.float32, (batch_size, self.num_labels))
        ...
        #newimg = (tf.tanh(newimg) + 1) / 2
        #newimg = newimg * (self.clip_max - self.clip_min) + self.clip_min

        l2dist_real = tf.reduce_sum(tf.square(tz), list(range(1, len(shape))))
